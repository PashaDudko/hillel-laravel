...
{{--                            <input type="hidden" name="new_quantity" class="new_quantity" value="17">--}}
                            <button id="plus" type="submit" class="btn border-2 btn-outline-dark" onclick="changeProductQuantityInCart('{{$item['slug']}}', 'plus', '{{$item['in_stock']}}');">+</button>
                            <span id="{{$item['slug']}}">{{$item['quantity']}}</span>
{{--                            <span class="{{$item['slug']}}">{{$item['quantity']}}</span>--}}
{{--                            <input type="number" id="current_quantity" value="{{$item['quantity']}}" min="0" max="10"> если тип поля намбер - то там уж есть счетчик и есть ограничения--}}
                            <button id="minus" type="submit" class="btn border-2 btn-outline-dark" onclick="changeProductQuantityInCart('{{$item['slug']}}', 'minus', '{{$item['in_stock']}}');">-</button>
                        </form>
                    </td>
                    <td>
...
@push('scripts')
    ...
    <script>
        ...
        function changeProductQuantityInCart(product_name, sign, in_stock) {
            ...
            // let current_quantity = Number(document.getElementsByClassName(product_name)[0].innerHTML);
            ...
            // document.getElementsByClassName(product_name)[0].innerHTML = current_quantity.toString();
            document.getElementById("new_quantity").value = current_quantity.toString();
            // document.getElementsByClassName("new_quantity")[0].value = current_quantity.toString();
